...
1. Hacer clic en el icono de Data Miner (`⛏️`) en el navegador

2. Seleccionar `▶️ Scrape This Page`

3. Ir a la sección `✏️ New Recipe` y seleccionar `➕ New Recipe` que desplegará el Recipe Creator

4. En la sección `Type` seleccionar `List / Search Page: multiple rows`

5. En la sección `Scrape` utilizar el `✏️ Easy Row Finder` para localizar cada una de las filas que conformarán la base de datos.

6. Una vez que identificaste las filas, asignar el tipo de información que tendrá cada una de ellas haciendo clic en `➕ Add New Column`. Para cada columna, define un nombre y un tipo de valor con ayuda del `✏️ Easy Column Finder`.

7. En la sección `Nav` ubicar el botón
